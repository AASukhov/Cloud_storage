...
public class FileController {
    ...
    @PutMapping()
    public ResponseEntity<?> editFileName(@RequestHeader("auth-token") String token, @RequestParam("filename") String filename, @RequestBody FileNameRequest fileNameRequest) {
        service.editFileName(token, filename, fileNameRequest);
        return ResponseEntity.ok(HttpStatus.OK);
    }
    @GetMapping("/all")
    public List<File> getAllFiles(@RequestHeader("auth-token") String  token, @RequestParam("limit") Integer limit) {
